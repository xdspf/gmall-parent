...
public class OrderStockDeduceListener {
    ...
    @RabbitListener(bindings = {
            @QueueBinding(
                    value = @Queue(name = MqConst.QUEUE_WARE_ORDER, durable = "true",exclusive = "false",autoDelete = "false"),
                    exchange = @Exchange(name = MqConst.EXCHANGE_WARE_ORDER, durable = "true",autoDelete = "false",type = "direct"),
    ...
    public void stockDeduceListener(Message message, Channel channel) throws IOException {
        ...
        try {
            ...
            orderInfoService.changeOrderStatus(orderId, orderInfo.getUserId(), status, Arrays.asList(ProcessStatus.PAID)
            );
